...
def run_protoc_with_plugin(
    ...
    # Add plugin options if provided
    if plugin_options:
        cmd.append(f"--py-mcp_opt={plugin_options}")
    ...
    return subprocess.run(cmd, capture_output=True, text=True, cwd=output_dir.parent)


def create_test_proto_request(
    proto_content: str, filename: str = "test.proto"
...
class TempProtoProject:
    ...
    def run_plugin(
        self, proto_files: Optional[List[str]] = None, plugin_options: Optional[str] = None
    ) -> subprocess.CompletedProcess:
